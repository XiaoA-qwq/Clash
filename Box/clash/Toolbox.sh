...
Clash_PATH="/data/adb/modules/Clash/"
MODULE_PROP="${Clash_PATH}module.prop"
GXClash_PATH="/data/adb/modules_update/Clash"
NET_PATH="/data/misc/net"
CTR_PATH="/data/misc/net/rt_tables"
SCRIPTS_PATH="/data/adb/Box/scripts/"
BOX_PATH="/data/adb/Box/scripts/box.config"
CONFIG_PATH="/data/adb/Box/clash/config.yaml"
CORE_PATH="/data/adb/Box/bin/clash"
COREE_PATH="/data/adb/Box/clash/"
VAR_PATH="/data/adb/Box/variab/"
...
PANEL_DIR="/data/adb/Box/panel/"
META_DIR="${PANEL_DIR}Meta/"
CHANGELOG_URL="https://raw.githubusercontent.com/XiaoA-qwq/Clash/main/changelog.md"
...
TEMP_FILE="/data/local/tmp/Clash_update.zip"
TEMP_DIR="/data/local/tmp/Clash_update"
DB_PATH="/data/adb/Box/clash/cache.db"
SERVICE_SCRIPT="/data/adb/Box/scripts/box.service"
CLASH_RELOAD_URL="http://127.0.0.1:9090/configs"
CLASH_RELOAD_PATH="/data/adb/Box/clash/config.yaml"
...
GEOIP_PATH="/data/adb/Box/clash/GeoIP.dat"
...
TOOLBOX_FILE="/data/adb/Box/clash/Toolbox.sh"
get_remote_version() {
...
update_module() {
    ...
    mv "$TEMP_DIR/Clash_service.sh" "$SERVICE_PATH"
    chmod 0700 "${SERVICE_PATH}/Clash_service.sh"

    if [ -d /data/adb/Box ]; then
        mkdir -p "$SCRIPTS_PATH"
        mkdir -p "$COREE_PATH"
        mkdir -p "$Clash_PATH"
        mkdir -p "$Clash_PATH/webroot"

        [ -f "$CONFIG_PATH" ] && mv "$CONFIG_PATH" "${CONFIG_PATH}.bak"
        [ -f "$BOX_PATH" ] && mv "$BOX_PATH" "${BOX_PATH}.bak"

        mv "$TEMP_DIR/Box/scripts/"* "$SCRIPTS_PATH"
        mv "$TEMP_DIR/Box/clash/config.yaml" "$COREE_PATH"

        find "$TEMP_DIR" -mindepth 1 -maxdepth 1 ! -name "README.md" ! -name "Clash_service.sh" ! -name "customize.sh" ! -name "Box" ! -name "META-INF" -exec cp -r {} "$Clash_PATH" \;

        [ -d "$TEMP_DIR/webroot" ] && cp -r "$TEMP_DIR/webroot/"* "$Clash_PATH/webroot/"

        restore_subscribe_urls
    else
        mkdir -p "$Clash_PATH"
        mv "$TEMP_DIR/Box" "/data/adb/"
        mv "$TEMP_DIR/webroot" "$Clash_PATH"
        find "$TEMP_DIR" -mindepth 1 -maxdepth 1 ! -name "README.md" ! -name "Clash_service.sh" ! -name "customize.sh" ! -name "Box" ! -name "META-INF" -exec cp -r {} "$Clash_PATH" \;
    fi

    chown -R root:net_admin /data/adb/Box/
    find /data/adb/Box/ -type d -exec chmod 755 {} \;
    find /data/adb/Box/ -type f -exec chmod 644 {} \;
    chmod -R 711 /data/adb/Box/scripts/
    chmod -R 700 /data/adb/Box/bin/
    ...
    nohup inotifyd "${SCRIPTS_PATH}box.inotify" "$Clash_PATH" > /dev/null 2>&1 &
...
integrate_magisk_update() {
    ...
    VARIAB_PATH="$Clash_PATH/variab"
    TEMP_PATH="/data/local/tmp/Clash_variab_backup"
    if [ -d "$GXClash_PATH" ]; then
        echo "检测到 安装/更新 Clash 模块，进行整合..."
        rm -rf "$Clash_PATH"
        mv "$GXClash_PATH" /data/adb/modules/
        if [ -f "$Clash_PATH/update" ]; then
            rm -f "$Clash_PATH/update"
        fi    
        echo "整合成功✓"
    else
        echo "没有检测到 安装/更新 Clash 模块。"
    fi
}
clear_cache() {
    if [ ! -f "$MODULE_PROP" ]; then
    ...
    echo "重启模块服务中..."
    touch "$Clash_PATH/disable"
    sleep 1.5
    rm -f "$Clash_PATH/disable"
    sleep 1.5
    echo "ok"
...
update_geo_database() {
    ...
    ensure_var_path
    echo "↴"
    if [ -f "/data/adb/Box/clash/geosite.dat" ]; then
        rm "/data/adb/Box/clash/geosite.dat"
    fi
    if [ -f "/data/adb/Box/clash/geoip.dat" ]; then
        rm "/data/adb/Box/clash/geoip.dat"
    fi
    echo "正在下载文件中..."
...
reload_configuration() {
    ...
    if [ -f "/data/adb/modules/Clash/disable" ]; then
        echo "↴" 
        echo "服务未运行，重载操作失败！"
        return
    fi
...
open_project_page() {
    ...
    if command -v xdg-open > /dev/null; then
        xdg-open "https://github.com/XiaoA-qwq/Clash"
    elif command -v am > /dev/null; then
        am start -a android.intent.action.VIEW -d "https://github.com/XiaoA-qwq/Clash"
    echo "ok" 
    else
        echo "无法打开浏览器，请手动访问以下地址："
        echo "https://github.com/XiaoA-qwq/Clash"
    fi
}
show_menu
